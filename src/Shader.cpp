...
void Shader::Compile(const char *vertexSource, const char *fragmentSource,
  ...
  glShaderSource(vS, 1, &fragmentSource, nullptr);
  glCompileShader(vS);
  checkCompilerErrors(vS, "FRAGMENT");

  if (geometrySource != nullptr) {
    gS = glCreateShader(GL_GEOMETRY_SHADER);
    glShaderSource(vS, 1, &geometrySource, nullptr);
    glCompileShader(vS);
    checkCompilerErrors(vS, "GEOMETRY");
  }
...
void Shader::SetMatrix4(const char *name, const glm::mat4 &matrix,
  ...
  glUniform4fv(glGetUniformLocation(this->ID, name), 1, glm::value_ptr(matrix));
}

void Shader::checkCompilerErrors(unsigned int object, std::string type) {
  int success;
  char infoLog[1024];
  if (type != "PROGRAM") {
